        ...
        <div class="container-fluid ml-1 mt-2 text-center" style="width: 100%">
            <form action="" class="form-horizontal" method="">
                ...
                <div class="row form-group mb-3">
                    ...
                    <div class="col-sm-2">
                        <input type="date" name="date" id="date" class="form-control" value="{{ old('date',$footerData->date) }}">
                    </div>

                    <label for="" class="col-sm-1 col-form-label">Tax</label>
                    <div class="col-sm-2">
                        <input type="number" name="tax" id="tax" class="form-control" value="{{ old('tax', $footerData->tax) }}">
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-10 border border-1" style="border-radius: 15px">
                        ...
                        <input type="hidden" name="vCode" id="vCode" value="{{ $footerData->voucher_code }}">
                        <input type="hidden" name="vId" id="vId" value="{{ $footerData->voucher_id }}">
                            ...
                            <table class="table table-hover" id="tblID">
                                ...
                                <tbody id="tblBody">
                                    @foreach ($detail as $item)
                                        <tr>
                                            <td>{{ $item->v_detail_id }}</td>
                                            <td>{{ $item->voucher_code }}</td>
                                            <td>{{ $item->date }}</td>
                                            <td>
                                                @foreach ($data as $vItem)
                                                    @if ($vItem->item_id == $item->item)
                                                        {{ $vItem->item }}
                                                    @endif
                                                @endforeach
                                            </td>
                                ...
                                <tfoot id="tblFoot">
                                    <tr id="trTax">
                                        ...
                                        <td id="tdTax">{{ $footerData->tax }}</td>
                                    </tr>
                                    <tr id="trTotal">
                                        <td colspan="8" style="text-align: right">Total</td>
                                        <td id="total">{{ $footerData->total }}</td>
                                    </tr>
                                </tfoot>
                            </table>
...
$(document).ready(function(e){
    ...
    $("body").on("click", ".btn-edit", function(e){
        ...
        $quantity = $(this).parents("tr").find("td:eq(6)").text();
        $discount = $(this).parents("tr").find("td:eq(7)").text();
    ...
    function save(){
        var $ary = [];
        var detailID, voucher_code, date, item, itemValue, price, quantity, discount, tax;
        var table = $("#tblBody");
        // $voucher_code = $("#vCode").val();
        table.find('tr').each(function(i){

            var $tds = $(this).find('td');
            detailID = $tds.eq(0).text();
            voucher_code = $tds.eq(1).text();
            date = $tds.eq(2).text();
            item = $tds.eq(3).text();
            ...
            subTotal = $tds.eq(8).text();
            tax = $("#tdTax").text();
            total = $("#total").text();
            $voucherDetail = {
                'v_detail_id' : detailID,
